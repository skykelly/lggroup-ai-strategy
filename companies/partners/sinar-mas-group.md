---
id: company_sinar_mas_group
type: company
title: Sinar Mas Group
category: partner
related_themes:
- ai_data_center_infra
- global_ai_alliance_open_innovation
related_topics: []
related_concepts: []
source_ids:
- src_sinar_mas_official
tags:
- sinar-mas-group
- partner
---

# Sinar Mas Group

<figure>
  <img src="assets/images/img_lguplus_paju_pulse_01.png" alt="Sinar Mas Group 관련 이미지" style="max-width:100%; border-radius:8px;" />
  <figcaption>LG Uplus Paju AIDC construction site</figcaption>
</figure>

## 1. Role Summary

인도네시아 AI 데이터센터 구축과 One LG 인프라 패키지의 해외 적용 후보로 계획 문서에 포함된 파트너다. docs/topics의 직접 근거와 구체적 계약 범위는 추가 검증이 필요하다.

## 2. Theme Participation

| Theme | Role | Evidence |
|---|---|---|
| [[docs/01_ai_data_center_infra|AI Data Center / Infra]] | LG CNS·LG전자·LG에너지솔루션 인프라 package의 해외 적용 파트너 후보 — 검증 필요 | `src_sinar_mas_official` |
| [[docs/06_global_ai_alliance_open_innovation|Global AI Alliance / Open Innovation]] | 동남아 AI 인프라 사업 확장을 위한 현지 사업 접점 — 검증 필요 | `src_sinar_mas_official` |

## 3. Topic Participation

| Topic | Why It Matters |
|---|---|
| 직접 연결된 Topic 없음 | 추가 검증 필요 |

## 4. Key Assets

- 인도네시아 사업 기반
- AI 데이터센터 수요·부지·고객 접점 가능성
- One LG Solution 해외 적용 후보

## 5. Related Concepts

- 관련 Concept 보강 필요

## 6. Related Sources

- [[sources/src_sinar_mas_official]]

## 7. Open Questions

- 이 회사가 lead해야 할 제품·서비스 단위와 다른 계열사 또는 파트너의 역할 경계는 무엇인가?
- PoC나 제휴 발표를 반복 가능한 매출·생산성·R&D 성과로 전환할 KPI는 무엇인가?
- 데이터, 모델, workflow와 고객 관계 중 반드시 내부에 축적해야 할 자산은 무엇인가?
