---
id: company_sk_enmove
type: company
title: SK Enmove
category: partner
related_themes:
- ai_data_center_infra
related_topics: []
related_concepts: []
source_ids:
- src_sk_enmove_official
tags:
- sk-enmove
- partner
---

# SK Enmove

<figure>
  <img src="https://upload.wikimedia.org/wikipedia/commons/5/53/Apple_datacenter_Reno_satellite_view.jpg" alt="SK Enmove 관련 이미지" style="max-width:100%; border-radius:8px;" />
  <figcaption>LG Electronics CDU on display</figcaption>
</figure>

## 1. Role Summary

데이터센터 액침냉각 유체와 열관리 ecosystem에 연결되는 국내 파트너·경쟁 사업자 후보로 문서에 포함돼 있다. 직접 협력 범위는 추가 검증이 필요하다.

## 2. Theme Participation

| Theme | Role | Evidence |
|---|---|---|
| [[docs/01_ai_data_center_infra|AI Data Center / Infra]] | 액침냉각 fluid와 열관리 value chain의 협력·경쟁 후보 — 검증 필요 | `src_sk_enmove_official` |

## 3. Topic Participation

| Topic | Why It Matters |
|---|---|
| 직접 연결된 Topic 없음 | 추가 검증 필요 |

## 4. Key Assets

- 열관리 유체
- Immersion cooling ecosystem
- 에너지 효율 솔루션

## 5. Related Concepts

- 관련 Concept 보강 필요

## 6. Related Sources

- [[sources/src_sk_enmove_official]]

## 7. Open Questions

- 이 회사가 lead해야 할 제품·서비스 단위와 다른 계열사 또는 파트너의 역할 경계는 무엇인가?
- PoC나 제휴 발표를 반복 가능한 매출·생산성·R&D 성과로 전환할 KPI는 무엇인가?
- 데이터, 모델, workflow와 고객 관계 중 반드시 내부에 축적해야 할 자산은 무엇인가?
