...
Knowledge Graph은(는) 객체와 관계 기반 지식 그래프을(를) 의미하며, Enterprise AX / Agentic Operating Model 전략을 설계하고 평가하는 개념으로 사용한다.
...
객체와 관계 기반 지식 그래프이라는 목적을 달성하려면 데이터 입력, 모델 또는 분석 계층, 업무·현장 시스템 연계, 실행 결과의 측정과 피드백이 연결되어야 한다. LG 전략에서는 기술 도입 여부보다 이 전체 loop가 반복 운영되고 내부 역량으로 축적되는지를 기준으로 본다.

## 4. LG Relevance

| 관련 테마 | 연결 이유 |
|---|---|
| [[docs/04_enterprise_ax_agentic_operating_model]] | **Knowledge Graph**: 객체와 관계 기반 지식 그래프. 기업 데이터와 업무 규칙을 의사결정·실행 가능한 Agent workflow로 바꾸는 운영 기반이다. |
| [[docs/06_global_ai_alliance_open_innovation]] | Knowledge Graph의 핵심 기능 — 객체와 관계 기반 지식 그래프 — 이 이 테마로 확장되면 자체 역량만으로 확보하기 어려운 모델·플랫폼·하드웨어를 파트너와 결합하되 의존도를 관리하게 한다. |

## 5. Related Companies

| 회사 | 관련 역할 |
|---|---|
| LG Corp. | 그룹 AX 우선순위, 공통 KPI, 데이터 공유 원칙과 계열사 간 의사결정 구조를 설계한다. Knowledge Graph에서는 ‘객체와 관계 기반 지식 그래프’ 기능의 계열사별 책임, 투자 기준, 그룹 공통 KPI를 조정한다. |
| LG CNS | ERP·SCM·MES·PLM 연계, AX Platform, Knowledge Lake, LLMOps와 Governance를 구현한다. Knowledge Graph에서는 ‘객체와 관계 기반 지식 그래프’ 기능을 데이터 파이프라인과 업무시스템에 구현하고 외부 고객용 구축 방법론으로 표준화한다. |
| LG AI Research | EXAONE·K-EXAONE과 Advanced Agent를 그룹 문서·업무 데이터에 맞게 고도화한다. Knowledge Graph에서는 ‘객체와 관계 기반 지식 그래프’ 기능에 필요한 모델 학습·평가 체계와 도메인 지능을 개발한다. |
| Palantir | Foundry/AIP와 ontology 운영 경험을 제공하지만 LG의 공통 데이터 모델 자체를 대신하는 주체는 아니다. Knowledge Graph에서는 ‘객체와 관계 기반 지식 그래프’ 기능을 ontology 기반 운영 모델로 구조화하고 초기 구현을 지원한다. |
