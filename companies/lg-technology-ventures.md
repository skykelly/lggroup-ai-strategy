---
id: company_lg_technology_ventures
type: company
title: LG Technology Ventures
category: lg_affiliate
related_themes:
- ai_for_science_bio_materials_battery
- global_ai_alliance_open_innovation
related_topics: []
related_concepts:
- global-ai-alliance
- open-innovation
- cvc
- ai-factory
- robotic-foundation-model
- enterprise-ontology
- physical-ai
- sdv
- battery-software
- ai-co-scientist
- hybrid-ai-strategy
source_ids:
- src_lg_nvidia_map_20260608
- src_nvidia_lg_ai_factory_20260607
- src_lgcorp_silicon_valley_20260407
- src_lgcns_palantir_20260312
- src_lgcns_skild_ai_20250616
- src_lgensol_qualcomm_bms_20241223
- src_lgensol_sdverse_20260403
- src_lgai_dd_pharmatech_20260617
tags:
- lg-technology-ventures
- lg-affiliate
---

# LG Technology Ventures

<figure>
  <img src="assets/images/lg_nvidia_map_koo_huang_prn.jpg" alt="LG Technology Ventures 관련 이미지" style="max-width:100%; border-radius:8px;" />
  <figcaption>LG Corp. 구광모 회장과 NVIDIA Jensen Huang CEO. M.A.P. 협력은 Mobility, AI Infra, Physical AI를 연결하는 6번 테마의 핵심 축이다.</figcaption>
</figure>

## 1. Role Summary

AI·로봇·바이오·소재 스타트업에 대한 전략적 투자로 기술 옵션을 확보하는 CVC 축이다. 구체적 건별 역할은 추가 검증이 필요하다.

## 2. Theme Participation

| Theme | Role | Evidence |
|---|---|---|
| [[docs/05_ai_for_science_bio_materials_battery|AI for Science / Bio / Materials / Battery]] | 바이오·소재 AI 스타트업 option 확보 — 검증 필요 | `src_lg_nvidia_map_20260608`, `src_nvidia_lg_ai_factory_20260607`, `src_lgcorp_silicon_valley_20260407` |
| [[docs/06_global_ai_alliance_open_innovation|Global AI Alliance / Open Innovation]] | CVC를 통한 외부 기술 탐색·투자·내부 연결 | `src_lg_nvidia_map_20260608`, `src_nvidia_lg_ai_factory_20260607`, `src_lgcorp_silicon_valley_20260407` |

## 3. Topic Participation

| Topic | Why It Matters |
|---|---|
| 직접 연결된 Topic 없음 | 추가 검증 필요 |

## 4. Key Assets

- 글로벌 스타트업 탐색
- 전략적 투자와 option portfolio
- 계열사 사업부와 기술 연결

## 5. Related Concepts

- [[concepts/global-ai-alliance]]
- [[concepts/open-innovation]]
- [[concepts/cvc]]
- [[concepts/ai-factory]]
- [[concepts/robotic-foundation-model]]
- [[concepts/enterprise-ontology]]
- [[concepts/physical-ai]]
- [[concepts/sdv]]
- [[concepts/battery-software]]
- [[concepts/ai-co-scientist]]
- [[concepts/hybrid-ai-strategy]]

## 6. Related Sources

- [[sources/src_lg_nvidia_map_20260608]]
- [[sources/src_nvidia_lg_ai_factory_20260607]]
- [[sources/src_lgcorp_silicon_valley_20260407]]
- [[sources/src_lgcns_palantir_20260312]]
- [[sources/src_lgcns_skild_ai_20250616]]
- [[sources/src_lgensol_qualcomm_bms_20241223]]
- [[sources/src_lgensol_sdverse_20260403]]
- [[sources/src_lgai_dd_pharmatech_20260617]]

## 7. Open Questions

- 이 회사가 lead해야 할 제품·서비스 단위와 다른 계열사 또는 파트너의 역할 경계는 무엇인가?
- PoC나 제휴 발표를 반복 가능한 매출·생산성·R&D 성과로 전환할 KPI는 무엇인가?
- 데이터, 모델, workflow와 고객 관계 중 반드시 내부에 축적해야 할 자산은 무엇인가?
